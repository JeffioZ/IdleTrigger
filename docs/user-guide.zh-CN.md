...
在“设置 > 屏幕提示”中开启锁定键状态提示。此功能默认关闭，可分别选择 Caps Lock、Num Lock 和 Scroll Lock；关闭总开关会保留各按键的选择。

提示会短暂显示在前台应用所在的屏幕上，不抢键盘焦点，也不拦截鼠标，动效遵循 Windows 动画设置。默认在检测到全屏应用或演示状态时隐藏，结束后不补弹期间的变化。此选项不影响系统操作前的倒计时预警。

“预览提示”会展示 Caps Lock 开启的示例，不改变真实按键状态，也不保存设置。应用文字同时响应 Windows 的文本大小设置；放大后的控制面板和设置窗口在空间不足时可滚动操作。
