...
| | 能力 | 适用场景 |
| --- | --- | --- |
| ⚡ | **保持唤醒** | 下载、渲染、备份或远程连接期间阻止自动睡眠；支持限时档位，到期自动恢复。 |
| ⏱️ | **空闲动作** | 真实键盘、鼠标持续无操作后，自动锁定、睡眠、休眠、关机、重启或熄屏。 |
| 🔁 | **自动任务** | 按模板快速创建，按计划或进程状态控制电源功能，也可执行内置系统动作。 |
| 🌗 | **昼夜主题** | 按时间或日出日落切换 Windows 主题，可联动壁纸与指针方案，可推迟单次切换，并适配电池和全屏场景。 |
